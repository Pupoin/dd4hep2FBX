...
#include <vector>
// #include "G4Types.hh"
#include <CLHEP/Vector/TwoVector.h> //#include "CLHEP::Hep2Vector.hh"
...
class HepPolyhedron {
 ...
 protected:
  static /*G4ThreadLocal*/ int fNumberOfRotationSteps;
  int nvert, nface;
  ...
  // Create HepPolyhedron for body of revolution around Z-axis
  void RotateContourAroundZ(int nstep, double phi, double dphi,
                            const std::vector<CLHEP::Hep2Vector> &rz,
                            int nodeVis, int edgeVis);

  // Triangulate closed polygon (contour)
  bool TriangulatePolygon(const std::vector<CLHEP::Hep2Vector> &polygon,
                            std::vector<int> &result);

  // Helper function for TriangulatePolygon()
  bool CheckSnip(const std::vector<CLHEP::Hep2Vector> &contour,
                   int a, int b, int c,
                   int n, const int* V);
 ...
 public:
  ...
  int GetNoVerteces() const { return nvert; }  // Old spelling.
  ...
  bool GetNextEdgeIndices(int & i1, int & i2, int & edgeFlag,
  ...
  bool GetNextEdgeIndices(int & i1, int & i2, int & edgeFlag) const;
  bool GetNextEdgeIndeces(int & i1, int & i2, int & edgeFlag) const
  {return GetNextEdgeIndices(i1,i2,edgeFlag);}  // Old spelling.

  // Get next edge
  bool GetNextEdge(HepGeom::Point3D<double> &p1, HepGeom::Point3D<double> &p2, int &edgeFlag) const;

  // Get next edge
  bool GetNextEdge(HepGeom::Point3D<double> &p1, HepGeom::Point3D<double> &p2, int &edgeFlag,
                     int &iface1, int &iface2) const;
...
class HepPolyhedronPgon : public HepPolyhedron
{
 public:
  HepPolyhedronPgon(double phi, double dphi, int npdv, int nz,
                    ...
                    const double *rmin,
                    const double *rmax);
  HepPolyhedronPgon(double phi, double dphi, int npdv,
                    const std::vector<CLHEP::Hep2Vector> &rz);
...
class HepPolyhedronPcon : public HepPolyhedronPgon
{
 public:
  HepPolyhedronPcon(double phi, double dphi, int nz,
                    ...
                    const double *rmin,
                    const double *rmax);
  HepPolyhedronPcon(double phi, double dphi,
                    const std::vector<CLHEP::Hep2Vector> &rz);
...
{
 public:
  HepPolyhedronTet(const double p0[3],
                   const double p1[3],
                   const double p2[3],
                   const double p3[3]);
...
{
 public:
  HepPolyhedronHyperbolicMirror(double a, double h, double r);
